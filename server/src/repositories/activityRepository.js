export function createActivityRepository(db) {
  return {
    ...
    countHabitCompletions(userId, habitId, date) {
      return db.prepare(`
        SELECT COUNT(*) AS count
        FROM habit_completions
        WHERE user_id = ? AND habit_id = ? AND completed_on = ?
      `).get(userId, habitId, date).count
    },

    insertHabitCompletion(userId, completion) {
      db.prepare(`
    ...
    completionDates(userId, habitId) {
      return db.prepare(`
    ...
    setGoalProgress(userId, id, current, completionAwarded, weekKey) {
      db.prepare(`
        UPDATE goals
    ...
    resetWeeklyGoals(userId, weekKey) {
      db.prepare(`
        UPDATE goals
    ...
    insertGoalProgress(userId, event) {
      db.prepare(`
    ...
    completionCountsByDate(userId, startDate) {
      return db.prepare(`
